...
class Url {
	...
	public function parseGet($getString) {
		...
		if (preg_match('/=/', $getString)) {
			$deliminator = strstr($getString, '&amp;')? '&amp;' : '&';
			foreach (explode($deliminator, $getString) as $arg) {
	...
	//Unify and redirect for www or non www, http or https, right /
	public function unifyUrl($siteUrl) {
		$requestUrl = $this->getUrlFromRequest();
		$actualUrl = $this->getUrlFromSite($siteUrl);
		if($requestUrl['url'] != $actualUrl['url'])
			$this->redirect($actualUrl['url']);
