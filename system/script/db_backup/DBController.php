...
class DBController extends WonClass {
	...
	*
	...
	public function update_db($sql_file) {
		
		$sql_query = fread(fopen($sql_file, 'r'), filesize($sql_file)) or die('problem ');
		
		$sql_query = $this->remove_remarks($sql_query);
		$sql_query = $this->split_sql_file($sql_query, ';');
		
		$tables = Won::get('DB')->query("
			SHOW TABLES
		");
		if ($tables->num_rows) {
			$table_names = array();
