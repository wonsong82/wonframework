...
$.fn.maskSlider=function(){
	...
	var gap			= args.gap		|| 0.2; //Time gap between two masks
	var box			= args.box		|| 13;//Default num of boxes (columns)
	var init		= args.initAnimation || false;//Wheather you start with the animation first, or with the static element of the first one
	...
	var timer	=	null;
	var hover	=	false;
	var	con		=	$('<div class="maskSlider"/>');	//Container that will have banner, loading, navs in it.
	...
	function next() {
		...
		banner.find('.mask').each(function(){ // Set both Images
			var mask = $(this);
			mask.empty();
			...
			element2.css({
				'left': ((-1*maskX)+width)+'px'
			}).appendTo(mask);
			
			if (mask.hasClass('odd')) {
				if (c!=-1) {
					element1.stop(true).animate({'left':((-1*maskX)-width)+'px'}, (speed-gap)*1000, 'easeOutQuart');
				}
				element2.stop(true).animate({'left':(-1*maskX)+'px'}, (speed-gap)*1000, 'easeOutQuart');
			}
			if (mask.hasClass('even')) {
				if (c!=-1) {
					element1.stop(true).animate({'left':((-1*maskX)-width)+'px'}, speed*1000, 'easeOutQuart');
				}
				element2.stop(true).animate({'left':(-1*maskX)+'px'}, speed*1000, 'easeOutQuart');
			}
		});
	...
	for (i=0; i<box; i+=2) { // Mask1
		$('<div class="mask odd" />').css({
			'width':maskWidth+'px',
	...
	for (i=1; i<box; i+=2) { // Mask2
		$('<div class="mask even" />').css({
			'width':maskWidth+'px',
			'height':maskHeight+'px',
			'overflow':'hidden',
			'position':'absolute',
